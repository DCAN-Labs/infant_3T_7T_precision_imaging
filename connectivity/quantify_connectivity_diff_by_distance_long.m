function quantify_connectivity_diff_by_distance_long(infolder, dconn3T, dconn7T, dist_mat)
...
[longx, longy]=find(distances>=100); %find connections that are further than 10cm apart
clear distances
remaining_data=zeros(size(data));
for i=1:size(longx,1)
remaining_data(longx(i,1),longy(i,1))=data(longx(i,1),longy(i,1));
end
clear data
...
for i=1:size(longx,1)
remaining_data(longx(i,1),longy(i,1))=data7(longx(i,1),longy(i,1));
end
clear data7
dconntri=tril(abs(remaining_data));
clear remaining_data
...
writetable(stats, ['/myfolder/connectivity_strength/' sub '_' ses '_' echo '_histogram_long.csv'])
